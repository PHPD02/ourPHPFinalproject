<?php
    ...
    if(isset($_POST["reset-requset-submit"])){
        ...
        $bin2hextoken =bin2hex($token);

        // $url= "http://localhost:3000/resetpassword?selector=".$selector."&validator=".bin2hex($token); 
        $url= "http://localhost:80/ourPHPFinalproject/kangProject/forgetpassword/resetpassword/create-new-passwordcopy.php?selector=".$selector."&validator=".bin2hex($token); 
...
// 進行發送郵件
        $to = $userEmail;
        $subject = "重設您的密碼";
        $message = '<p style="color:red">請透過下方連結進入，並重設您的密碼!:<br />'; //裡面還包超連結，所以</P>下去
        ...
        header ("Location: http://localhost:3000/sendresetemail"); // ?reset=success透過get變亮返還成功訊息
        
        // $_SESSION['selector']="$selector";
        // $_SESSION['bin2hextoken)']="$bin2hextoken";

        ob_start();
        setcookie("selector","$selector");
        setcookie("bin2hextoken","$bin2hextoken");
        ob_end_flush();



    }else{
        header("Location: http://localhost:3000/forgotpwd?reset=notsuccess");
        // echo ("驗證信發生錯誤請再次重試");
    }
