...
// echo $_SESSION['bin2hextoken'];
echo $_COOKIE["selector"];
    $selector = $_COOKIE["selector"];
    $validator = $_COOKIE["bin2hextoken"];
    if(isset($_POST["reset-password-submit"])){ //是否送出申請

        $password = $_POST["pwd"];
        $passwordRepeat = $_POST["pwd-repeat"];
        $url= "http://localhost:3000/resetpassword?newpwd=empty&selector=".$selector."&validator=".$validator; 
        $url2= "http://localhost:3000/resetpassword?newpwd=twopasswordnotthesame&selector=".$selector."&validator=".$validator; 
        ...
        if(!mysqli_stmt_prepare($statement,$sql)){ //沒有執行成功
            ...
            if(!$row = mysqli_fetch_assoc($result)){
                ...
                if($tokenCheck ==false){
                    ...
                    $sql ="SELECT * FROM usermember WHERE email=?";  //換表
                    ...
                    if(!mysqli_stmt_prepare($statement,$sql)){
                        ...
                        if(!$row = mysqli_fetch_assoc($result)){
                            ...
                            $sql = "UPDATE usermember SET password =? WHERE email=?";
                            $statement = mysqli_stmt_init($connection);
                            ...
                            if(!mysqli_stmt_prepare($statement,$sql)){
                                ...
                                if(!mysqli_stmt_prepare($statement,$sql)){
                                    ...
                                    mysqli_stmt_execute($statement);
                                    sleep(5);
                                    header("Location: http://localhost:3000/CompleteResetPwd");
                                 }                  
                            }
                        }
